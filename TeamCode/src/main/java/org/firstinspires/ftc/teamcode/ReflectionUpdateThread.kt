...
abstract class ReflectionUpdateThread : OpMode() {
    ...
    private val vClawLeft: virtualRobot.hardware.Servo? = null
    private val vClawRight: virtualRobot.hardware.Servo? = null
    ...
    override fun init() {
        ...
        try {
            ...
            for (f in fields) {
                name = f.getName()
                if (name[0] == 'v' && Character.isUpperCase(name[1])) {
                    f.set(this, SallyJoeBot::class.java!!.getMethod("get" + name.substring(1)).invoke(null))
                }
            }
